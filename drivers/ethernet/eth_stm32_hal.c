...
#define PHY_SCSR			 ((uint16_t)0x001FU)  /*!< LAN8742 PHY Special Control/Status Register */
#define PHY_AUTONEGO_ENABLE		 ((uint16_t)0x1000U)  /*!< LAN8742 Auto-negotiation enable bit */
#define PHY_HCDSPEEDMASK		 ((uint16_t)0x001CU)  /*!< LAN8742 High Capability Speed Mask */
#define PHY_10BT_HD 		 	 ((uint16_t)0x0004U)  /*!< LAN8742 10Base-T half-duplex capability */
#define PHY_10BT_FD 		 	 ((uint16_t)0x0014U)  /*!< LAN8742 10Base-T full-duplex capability */
#define PHY_100BTX_HD		 	 ((uint16_t)0x0008U)  /*!< LAN8742 100Base-TX half-duplex capability */
#define PHY_100BTX_FD		 	 ((uint16_t)0x0018U)  /*!< LAN8742 100Base-TX full-duplex capability */
#define PHY_TIMEOUT 		 	 (5000U)              /*!< LAN8742 PHY operation timeout in milliseconds */

#define PHY_STATUS_LINK_DOWN		 ((int32_t) 1)        /*!< LAN8742 Link down status */
#define PHY_STATUS_100MBITS_FULLDUPLEX	 ((int32_t) 2)        /*!< LAN8742 100 Mbps full-duplex status */
#define PHY_STATUS_100MBITS_HALFDUPLEX	 ((int32_t) 3)        /*!< LAN8742 100 Mbps half-duplex status */
#define PHY_STATUS_10MBITS_FULLDUPLEX	 ((int32_t) 4)        /*!< LAN8742 10 Mbps full-duplex status */
#define PHY_STATUS_10MBITS_HALFDUPLEX	 ((int32_t) 5)        /*!< LAN8742 10 Mbps half-duplex status */
#define PHY_STATUS_AUTONEGO_NOTDONE	 ((int32_t) 6)        /*!< LAN8742 Auto-negotiation not completed */
#endif
...
static uint32_t eth_phy_get_link_state(ETH_HandleTypeDef *heth)
{
	...
	if ((readval & PHY_LINKED_STATUS) == 0)
	{
		...
		LOG_INF("Return Link Down status");
	...
	if ((readval & PHY_AUTONEGO_ENABLE) != PHY_AUTONEGO_ENABLE)
	{
		...
		if ((HAL_ETH_WritePHYRegister(heth, PHY_ADDR, PHY_BCR, PHY_AUTONEGO_ENABLE)) != HAL_OK)
		{
			return HAL_ERROR;
		}
	}
...
static void get_auto_nego_speed_duplex(ETH_HandleTypeDef *heth, uint32_t* speed, uint32_t* duplex)
{
	...
	}while((phyLinkState <= PHY_STATUS_LINK_DOWN) && ((HAL_GetTick() - tickstart) < PHY_TIMEOUT));
...
static int eth_enable_api_v2(const struct device *dev)
{
	...
	struct eth_stm32_hal_dev_data *dev_data;
	ETH_HandleTypeDef *heth;
	uint32_t speed = ETH_SPEED_100M;  // Initialize with default value
	uint32_t duplex = ETH_SPEED_100M;  // Initialize with default value
...
#if defined(CONFIG_ETH_STM32_AUTO_NEGOTIATION_ENABLE)
	/* Auto Nego enabled */
	get_auto_nego_speed_duplex(heth, &speed, &duplex);
